 ...
 * @version 2.0
 */
public class HTTPBasicAuthenticatorCachedBenchmark extends TestCase {
    ...
    /**
     * The TimeoutCacheFactory config namespace.
     */
    private static final String NAMESPACE2 = TimeoutCacheFactory.class.getName();
    
    /**
     * The DefaultKeyConverter config namespace.
     */
    private static final String NAMESPACE3 = DefaultKeyConverter.class.getName();
    
    /**
     * The HTTPBasicAuthenticatorBenchmark config namespace.
     */
    private static final String NAMESPACE4 = "com.topcoder.security.authenticationfactory.stresstests";
    ...
    public void setUp() throws Exception {
        ...
        cm.add(NAMESPACE, "stress2.xml", ConfigManager.CONFIG_MULTIPLE_XML_FORMAT);
        
        if (cm.existsNamespace(NAMESPACE2)) {
        	cm.removeNamespace(NAMESPACE2);
        }
        cm.add(NAMESPACE2, "stress2.xml", ConfigManager.CONFIG_MULTIPLE_XML_FORMAT);
        
        if (cm.existsNamespace(NAMESPACE3)) {
        	cm.removeNamespace(NAMESPACE3);
        }
        cm.add(NAMESPACE3, "stress2.xml", ConfigManager.CONFIG_MULTIPLE_XML_FORMAT);
        
        authenticator = new HTTPBasicAuthenticator(NAMESPACE);
        
        if (cm.existsNamespace(NAMESPACE4)) {
        	cm.removeNamespace(NAMESPACE4);
        }
        cm.add(NAMESPACE4, "stress_params.xml", ConfigManager.CONFIG_MULTIPLE_XML_FORMAT);
    }
    ...
    public void testAuthenticate() throws Exception {                
        ...
        System.out.println(count * 1000 / TIMEOUT + " cached authentications / second");
    }
    
    private String getProp(String prop) throws Exception {
        return (String) ConfigManager.getInstance().getProperty(NAMESPACE4, prop);
    }
