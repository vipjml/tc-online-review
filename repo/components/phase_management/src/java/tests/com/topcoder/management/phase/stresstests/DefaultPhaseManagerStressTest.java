...
public class DefaultPhaseManagerStressTest extends TestCase {
    ...
    protected void setUp() throws Exception {
        manager = new DefaultPhaseManager(new StressMockPhasePersistence("true"), StressMockIDGenerator.DEFAULT);
    ...
    /**
     * <p>
     * Stress test for DefaultPhaseManager#DefaultPhaseManager(String).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testCtor_namespace() throws Exception {
        ...
        for (int i = 0; i < RUN_TIMES; i++) {
            assertNotNull("Failed to create DefaultPhaseManager.", new DefaultPhaseManager("stress_test_ns"));
        }
        long end = System.currentTimeMillis();
        System.out.println("Testing DefaultPhaseManager(String) for " + RUN_TIMES + " times costs " + (end - start)
            + "ms");
    }

    /**
     * <p>
     * Stress test for DefaultPhaseManager#DefaultPhaseManager(PhasePersistence, IDGenerator).
     * </p>
     */
    public void testCtor() {
        long start = System.currentTimeMillis();
        for (int i = 0; i < RUN_TIMES; i++) {
            assertNotNull("Failed to create DefaultPhaseManager.", new DefaultPhaseManager(
                new StressMockPhasePersistence(""), StressMockIDGenerator.DEFAULT));
        }
        long end = System.currentTimeMillis();
    ...
    /**
     * <p>
     * Stress test for DefaultPhaseManager#updatePhases(Project, String).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testUpdatePhases() throws Exception {
        ...
        long end = System.currentTimeMillis();
        System.out.println("Testing updatePhases(Project, String) for " + RUN_TIMES + " times costs " + (end - start)
            + "ms");
    }

    /**
     * <p>
     * Stress test for DefaultPhaseManager#canStart(Phase).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testCanStart() throws Exception {
        ...
        manager.registerHandler(new StressMockPhaseHandler(OperationCheckResult.SUCCESS), type1,
            PhaseOperationEnum.START);
        manager.registerHandler(new StressMockPhaseHandler(new OperationCheckResult(false, "false message")), type2,
            PhaseOperationEnum.START);
        phase.setPhaseType(type1);
        long start = System.currentTimeMillis();
        for (int i = 0; i < RUN_TIMES; i++) {
            assertTrue("Should return true.", manager.canStart(phase).isSuccess());
        }
        long end = System.currentTimeMillis();
        ...
        for (int i = 0; i < RUN_TIMES; i++) {
            assertFalse("Should return false.", manager.canStart(phase).isSuccess());
        }
    ...
    public void testCanStart_NonHandle() throws Exception {
        ...
        long start = System.currentTimeMillis();
        for (int i = 0; i < RUN_TIMES; i++) {
            Assert.assertTrue(manager.canStart(phase).isSuccess());
        }
        long end = System.currentTimeMillis();
    ...
    /**
     * <p>
     * Stress test for DefaultPhaseManager#start(Phase, String).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testStart() throws Exception {
        ...
        long end = System.currentTimeMillis();
        System.out.println("Testing start(Phase, String) for " + RUN_TIMES + " times costs " + (end - start) + "ms");
    }

    /**
     * <p>
     * Stress test for DefaultPhaseManager#canEnd(Phase).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testCanEnd() throws Exception {
        ...
        manager.registerHandler(new StressMockPhaseHandler(OperationCheckResult.SUCCESS), type1, PhaseOperationEnum.END);
        manager.registerHandler(new StressMockPhaseHandler(new OperationCheckResult(false, "false message")), type2,
            PhaseOperationEnum.END);
        phase.setPhaseType(type1);
        for (int i = 0; i < RUN_TIMES; i++) {
            assertTrue("Should return true.", manager.canEnd(phase).isSuccess());
        }
        phase.setPhaseType(type2);
        long start = System.currentTimeMillis();
        for (int i = 0; i < RUN_TIMES; i++) {
            assertFalse("Should return false.", manager.canEnd(phase).isSuccess());
        }
        long end = System.currentTimeMillis();
        System.out.println("Testing canEnd(Phase) for " + RUN_TIMES + " times costs " + (end - start) + "ms");
    }
    ...
    public void testCanEnd_NonHandle() throws Exception {
        ...
        long end = System.currentTimeMillis();
        System.out.println("Testing canEnd(Phase) which has no corresponding handles for " + RUN_TIMES
            + " times costs " + (end - start) + "ms");
    }

    /**
     * <p>
     * Stress test for DefaultPhaseManager#end(Phase, String).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testEnd() throws Exception {
        ...
        long end = System.currentTimeMillis();
        System.out.println("Testing end(Phase, String) for " + RUN_TIMES + " times costs " + (end - start) + "ms");
    }

    /**
     * <p>
     * Stress test for DefaultPhaseManager#registerHandler(PhaseHandler, PhaseType,
     * PhaseOperationEnum) and DefaultPhaseManager#unregisterHandler(PhaseType, PhaseOperationEnum).
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testRegisterAndUnregisterHandler() throws Exception {
        ...
        for (int i = 0; i < RUN_TIMES; i++) {
            manager.registerHandler(new StressMockPhaseHandler(OperationCheckResult.SUCCESS), type1,
                PhaseOperationEnum.START);
            manager.registerHandler(new StressMockPhaseHandler(new OperationCheckResult(false, "false message")),
                type2, PhaseOperationEnum.END);
    ...
    /**
     * <p>
     * Stress test for DefaultPhaseManager#testGetAllHandlers().
     * </p>
     *
     * @throws Exception
     *             to junit.
    ...
    public void testGetAllHandlers() throws Exception {
        for (int i = 0; i < DATA_COUNT; i++) {
            manager.registerHandler(new StressMockPhaseHandler(OperationCheckResult.SUCCESS), new PhaseType(i + 100,
                "phasetype"), PhaseOperationEnum.START);
        }
        long start = System.currentTimeMillis();
        for (int i = 0; i < RUN_TIMES; i++) {
        ...
        long end = System.currentTimeMillis();
        System.out.println("Testing getAllHandlers() for " + RUN_TIMES + " times costs " + (end - start) + "ms");
    }

    /**
     * <p>
     * Stress test for DefaultPhaseManager#getHandlerRegistrationInfo(PhaseHandle).
     * </p>
     */
    public void testGetHandlerRegistrationInfo() {
        PhaseHandler handle = new StressMockPhaseHandler(OperationCheckResult.SUCCESS);
        for (int i = 0; i < DATA_COUNT; i++) {
            manager.registerHandler(handle, new PhaseType(i + 100, "phasetype"), PhaseOperationEnum.START);
        }
    ...
    private class StressMockPhaseHandler implements PhaseHandler {
        ...
        private OperationCheckResult canPerform;

        /**
         * Default constructor performs canPerform initialization.
         *
         * @param canPerform
         *            the boolean value to be set to this.canPerform.
         */
        public StressMockPhaseHandler(OperationCheckResult canPerform) {
