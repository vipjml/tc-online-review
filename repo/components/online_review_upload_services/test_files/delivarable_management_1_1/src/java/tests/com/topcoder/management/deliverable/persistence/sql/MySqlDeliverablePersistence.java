...
/**
 * <p> The SqlDeliverablePersistence class implements the DeliverablePersistence interface, in order to persist to the
 * database structure given in the deliverable_management.sql script. </p> <p> This class does not cache a Connection to
 * the database. Instead a new Connection is used on every method call. PreparedStatements should be used to execute the
 * SQL statements. </p> <p> Thread Safety: This class is immutable and thread-safe in the sense that multiple threads
 * can not corrupt its internal data structures. However, the results if used from multiple threads can be unpredictable
 * as the database is changed from different threads. This can equally well occur when the component is used on multiple
 * machines or multiple instances are used, so this is not a thread-safety concern. </p>
 *
...
public class MySqlDeliverablePersistence implements DeliverablePersistence {
    ...
    private static final String LOAD_DELIVERABLES_WITH_SUBMISSION_SQL = "SELECT "
            ...
            + " resource.project_id AND project_phase.phase_type_id = deliverable_lu.phase_type_id "
     ...
     *
     * @param connectionFactory The connection factory to use for getting connections to the database.
     * @param connectionName    The name of the connection to use. Can be null, which means default connection name will
     *                          be used.
     *
     ...
     *
     * @return The matching deliverables (possibly expanded by matching with each active submission, if it is a "per
     ...
     * submission" deliverable and the given submission must be "Active". If this is not the case, null is returned.
     * </p>
     *
     * @param deliverableId The id of the deliverable
     * @param resourceId    The resource id of deliverable
    ...
    /**
     * <p> Loads all Deliverables with the given ids and resource ids from persistence. May return a 0-length array.
     * </p>
     *
    ...
    /**
     * <p> Loads the deliverables associated with the given submissions and resource. The deliverables must be "per
     * submission" deliverables and the given submissions must be "Active". Pairs of ids not meeting this requirement
    ...
    /**
     * <p> Loads the deliverables which are NOT associated with submissions. The deliverables must NOT be "per
     * submission" deliverables. An additional match condition that much be meet is added to the tail of the sql string.
     * </p>
     *
    ...
    /**
     * <p> Loads the deliverables associated with the given submissions. The deliverables must be "per submission"
     * deliverables and the given submissions must be "Active". An additional match condition that much be meet is added
     * to the tail of the sql string. </p>
     *
     * @param conn           the connection to the persistence
     * @param matchCondition the addition condition
     *
