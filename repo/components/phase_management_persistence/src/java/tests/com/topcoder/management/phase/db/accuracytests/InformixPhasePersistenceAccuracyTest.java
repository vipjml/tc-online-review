 ...
 * @version 1.0
 */
public class InformixPhasePersistenceAccuracyTest extends TestCase {
    /**
    ...
    protected void setUp() throws Exception {
        ...
        AccuracyTestsHelper.loadConfigurations();
        AccuracyTestsHelper.executeDBScript(AccuracyTestsHelper.SCRIPT_FILL);

        generator = IDGeneratorFactory.getIDGenerator("phase_id_seq");

        persistence = new InformixPhasePersistence(new DBConnectionFactoryImpl(
                AccuracyTestsHelper.CONNECTION_FACTORY_NAMESPACE), "informix", generator);
    }
    ...
    protected void tearDown() throws Exception {
        AccuracyTestsHelper.executeDBScript(AccuracyTestsHelper.SCRIPT_CLEAR);
        AccuracyTestsHelper.closeAllConnections();
        AccuracyTestsHelper.clearAllConfigurations();
    ...
    public void testConstructor() throws Exception {
        try {
            persistence = new InformixPhasePersistence(AccuracyTestsHelper.PHASE_PERSISTENCE_NAMESPACE);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * Test <code>getProjectPhases(long)</code> for a simply project. There is no dependency
     * relationship between the dependencies.
     * <p>
    ...
    public void testGetProjectPhasesLong_Simple() throws Exception {
        ...
        assertEquals("Failed to set startDate",
                parseDate("2006-01-01 11:11:11.111"), project.getStartDate());
        ...
        assertEquals("Failed to set fixedStartTime.",
                parseDate("2006-01-01 11:11:11.222"), phase.getFixedStartDate());
        assertEquals("Failed to set scheduledStartTime",
                parseDate("2006-01-01 11:11:11.333"), phase.getScheduledStartDate());
        assertEquals("Failed to set scheduledEndTime",
                parseDate("2006-01-01 11:11:22.444"), phase.getScheduledEndDate());
        assertEquals("Failed to set actualStartTime",
                parseDate("2006-01-01 11:11:11.555"), phase.getActualStartDate());
        assertEquals("Failed to set actualEndTime",
                parseDate("2006-01-01 11:11:11.666"), phase.getActualEndDate());
        assertEquals("Failed to set length", 6000, phase.getLength());
    }

    /**
     * Test <code>getProjectPhases(long)</code> for a simply project. There are some depedencies
     * between the phases.
     * <p>
    ...
    public void testGetProjectPhasesLong_Complex() throws Exception {
        ...
        assertEquals("Phase 1 should have no dependency.",
                0, phases[0].getAllDependencies().length);

        assertEquals("Phase 2 should have 1 dependency.",
                1, phases[1].getAllDependencies().length);
        assertEquals("Phase 2 should depend on Phase 1.",
                1, phases[1].getAllDependencies()[0].getDependency().getId());
        assertEquals("Invalid dependency lagtime between 2 and 1.",
                12000, phases[1].getAllDependencies()[0].getLagTime());
        assertEquals("Invalid dependencyStart lagtime between 2 and 1.",
                true, phases[1].getAllDependencies()[0].isDependencyStart());
        assertEquals("Invalid dependencyStart lagtime between 2 and 1.",
                false, phases[1].getAllDependencies()[0].isDependentStart());

        assertEquals("Phase 3 should have 1 dependency.",
                1, phases[2].getAllDependencies().length);
        assertEquals("Phase 3 should depend on Phase 2.",
                2, phases[2].getAllDependencies()[0].getDependency().getId());
        assertEquals("Invalid dependency lagtime between 3 and 2.",
                24000, phases[2].getAllDependencies()[0].getLagTime());

        assertEquals("Phase 4 should have 1 dependency.",
                1, phases[1].getAllDependencies().length);
        assertEquals("Phase 4 should depend on Phase 1.",
                1, phases[1].getAllDependencies()[0].getDependency().getId());
    }
    ...
    public void testGetProjectPhasesLongArray_Special() throws Exception {
        ...
        assertEquals("Should return empty project.",
                0, projects[1].getAllPhases().length);
    }
    ...
    public void testCreatePhase_Normal() throws Exception {
        ...
        // validate the result
        DBRecord record = AccuracyTestsHelper.getPhaseRecords(" WHERE project_phase_id=" + phase.getId())[0];
        assertEquals("Failed to store project_id", new Long(1), record.getValue("project_id"));
        assertEquals("Failed to store fixedStartDate",
                phase.getFixedStartDate(), record.getValue("fixed_start_time"));
        assertEquals("Failed to store scheduledStartDate",
                phase.getScheduledStartDate(), record.getValue("scheduled_start_time"));
        assertEquals("Failed to store scheduledEndDate",
                phase.getScheduledEndDate(), record.getValue("scheduled_end_time"));
        assertEquals("Failed to store actualStartDate",
                phase.getActualStartDate(), record.getValue("actual_start_time"));
        assertEquals("Failed to store actualEndDate",
                phase.getActualEndDate(), record.getValue("actual_end_time"));
        assertEquals("Failed to store phase status id",
                new Long(2), record.getValue("phase_status_id"));
        assertEquals("Failed to store phase type id",
                new Long(1), record.getValue("phase_type_id"));

        // validate the dependency
        DBRecord[] records = AccuracyTestsHelper.getDependencyRecords(" WHERE dependent_phase_id=" + phase.getId());
        ...
        assertEquals("Failed to store dependencyStart.",
                new Boolean(false), record.getValue("dependency_start"));
        assertEquals("Failed to store dependentStart.",
                new Boolean(true), record.getValue("dependent_start"));
        assertEquals("Failed to store lagtime.",
                new Long(7654321), record.getValue("lag_time"));
    }
    ...
    public void testCreatePhase_Special() throws Exception {
        ...
        // validate the result
        DBRecord record = AccuracyTestsHelper.getPhaseRecords(" WHERE project_phase_id=" + phase.getId())[0];
        assertEquals("Failed to store fixedStartDate as null",
                phase.getFixedStartDate(), record.getValue("fixed_start_time"));
        assertEquals("Failed to store scheduledStartDate",
                phase.getScheduledStartDate(), record.getValue("scheduled_start_time"));
        assertEquals("Failed to store scheduledEndDate",
                phase.getScheduledEndDate(), record.getValue("scheduled_end_time"));
        assertEquals("Failed to store actualStartDate as null",
                phase.getActualStartDate(), record.getValue("actual_start_time"));
        assertEquals("Failed to store actualEndDate as null",
                phase.getActualEndDate(), record.getValue("actual_end_time"));
        assertEquals("Failed to store phase status id",
                new Long(2), record.getValue("phase_status_id"));
        assertEquals("Failed to store phase type id",
    ...
    public void testCreatePhases_Normal() throws Exception {
        ...
        DBRecord[] phaseRecords = AccuracyTestsHelper.getPhaseRecords(
                " WHERE create_user=\'testCreatePhases_Normal\' ORDER BY project_phase_id ASC");
        assertEquals("Failed to create all phases.",
                phases.length, phaseRecords.length);
        assertEquals("Failed to store fixed_start_time.",
                phases[1].getFixedStartDate(), phaseRecords[1].getValue("fixed_start_time"));
        assertEquals("Failed to store phase length.",
                new Long(phases[0].getLength()), phaseRecords[0].getValue("duration"));
        assertEquals("Failed to store phase type.",
                new Long(phases[2].getPhaseStatus().getId()), phaseRecords[2].getValue("phase_status_id"));

        // validate the dependency
        DBRecord[] dependencyRecords = AccuracyTestsHelper.getDependencyRecords(
                " WHERE dependent_phase_id=" + phases[0].getId() + " ORDER BY dependency_phase_id ASC");
        assertEquals("Should contain 2 dependencies.", 2, dependencyRecords.length);
        assertEquals("Failed add dependency with old record.",
                new Long(3), dependencyRecords[0].getValue("dependency_phase_id"));
        assertEquals("Failed add dependency with old record.",
                new Long(phases[1].getId()), dependencyRecords[1].getValue("dependency_phase_id"));
    }
    ...
    /**
     * Test <code>getPhase(long)</code> for a complex phase. There is some dependencies
     * associated with it.
     * <p>
    ...
    public void testGetPhase_Simple() throws Exception {
        ...
        assertEquals("Failed to get scheduledStartTime",
                parseDate("2006-01-01 11:11:11.111"), phase.getScheduledStartDate());
        assertEquals("Failed to get scheduledEndTime",
                parseDate("2006-01-01 11:11:22.111"), phase.getScheduledEndDate());
        ...
        assertEquals("There should be 2 phases of the project.",
                2, project.getAllPhases().length);
    }

    /**
     * Test <code>getPhase(long)</code> for a simply phase. There is no dependency
     * associated with it.
     * <p>
     * In this case, we mainly focus on whether the fields of phase is retrieved properly.
     * </p>
    ...
    public void testGetPhase_Complex() throws Exception {
        ...
        // chech the dependencies between phases.
        assertEquals("Phase 2 should have 1 dependency.",
                1, phase.getAllDependencies().length);
        assertEquals("Phase 2 should depend on Phase 1.",
                1, phase.getAllDependencies()[0].getDependency().getId());
        assertEquals("Invalid dependency lagtime between 2 and 1.",
                12000, phase.getAllDependencies()[0].getLagTime());
        assertEquals("Invalid dependencyStart lagtime between 2 and 1.",
                true, phase.getAllDependencies()[0].isDependencyStart());
        assertEquals("Invalid dependencyStart lagtime between 2 and 1.",
                false, phase.getAllDependencies()[0].isDependentStart());

        assertEquals("Phase 1 should have no dependency.",
                0, phase.getAllDependencies()[0].getDependency().getAllDependencies().length);

        assertEquals("The project of phases should be same.",
                phase.getProject(), phase.getAllDependencies()[0].getDependency().getProject());
    }
    ...
    public void testGetPhases_Normal() throws Exception {
        ...
        // verify the results
        assertEquals("Phase 5 and 6 should have same project.",
                phases[0].getProject().getId(), phases[3].getProject().getId());
        assertEquals("Phase 2 and 3 should have same project.",
                phases[1].getProject().getId(), phases[2].getProject().getId());

        assertEquals("project 2 should have 2 phases.",
                2, phases[0].getProject().getAllPhases().length);
        assertEquals("project 1 should have 4 phases.",
                4, phases[2].getProject().getAllPhases().length);

        assertEquals("There is no dependency with phase 5.",
                0, phases[0].getAllDependencies().length);
        assertEquals("Phase 3 should depend on phase 2.",
                phases[1], phases[2].getAllDependencies()[0].getDependency());
    }
    ...
    public void testUpdatePhase_Normal() throws Exception {
        ...
        // validate the dependency
        DBRecord[] records = AccuracyTestsHelper.getDependencyRecords(" WHERE dependent_phase_id = 4");
        assertEquals("Should store 2 dependencies.", 2, records.length);
        record = null;
        for (int i = 0; i < records.length; ++i) {
            if (records[i].getValue("dependency_phase_id").equals(new Long(3))) {
        ...
        assertEquals("Failed to store dependentStart.",
                new Boolean(true), record.getValue("dependent_start"));
        assertEquals("Failed to store lagtime.",
                new Long(1234567), record.getValue("lag_time"));
    }
    ...
    public void testUpdatePhase_NotExist() throws Exception {
        ...
        // validate the result
        DBRecord[] records = AccuracyTestsHelper.getDependencyRecords("");
    ...
    public void testUpdatePhases_Normal() throws Exception {
        ...
        DBRecord[] records = AccuracyTestsHelper.getDependencyRecords(
                " WHERE dependent_phase_id=3");
        assertEquals("Should remove one and add one dependency.", 1, records.length);
        assertEquals("Should update the dependency.",
                new Long(1), records[0].getValue("dependency_phase_id"));
    }
    ...
    public void testDeletePhase_Normal() throws Exception {
        ...
        DBRecord[] records = AccuracyTestsHelper.getPhaseRecords(" WHERE project_phase_id=" + phase.getId());
        assertEquals("The phase should be deleted.", 0, records.length);

        // check whether the dependency is deleted.
        records = AccuracyTestsHelper.getDependencyRecords(
                " WHERE dependency_phase_id=2 OR dependent_phase_id=2");
    ...
    public void testDeletePhase_NotExist() throws Exception {
        ...
        // check whether the phase is deleted.
        DBRecord[] records = AccuracyTestsHelper.getPhaseRecords("");
    ...
    public void testDeletePhases_SomeNotExist() throws Exception {
        ...
        // check whether the phase is deleted.
        DBRecord[] records = AccuracyTestsHelper.getPhaseRecords(" WHERE project_phase_id=" + phases[3].getId());
        assertEquals("The phase should be deleted.", 0, records.length);

        // check whether the dependency is deleted.
        records = AccuracyTestsHelper.getDependencyRecords(
                " WHERE dependency_phase_id=1 OR dependent_phase_id=" + phases[3].getId());
        assertEquals("Failed to remove dependencies.", 0, records.length);
    }
    ...
    public void testIsNewPhase_OldPhase() throws Exception {
        ...
        assertEquals("Should return false for old phase.",
                false, persistence.isNewPhase(phase));
    }
    ...
    public void testIsNewPhase_NewPhase() throws Exception {
        ...
        assertEquals("Should return true for new phase.",
                true, persistence.isNewPhase(phase));
    }

    /**
     * Test <code>isNewDependency(Dependency)</code>.
     * <p>
     * Since this method is not implemented before final fix.
     * This case is always successful.
     * </p>
     */
