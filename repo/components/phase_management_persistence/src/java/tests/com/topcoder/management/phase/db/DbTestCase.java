...
class DbTestCase extends TestCase {
    ...
    /**
     * Creats a project with given id.
     *
    ...
    protected void assertDatabase(Phase expected, String operator) throws Exception {
        ...
        try {
            ...
            if (expected.getAllDependencies().length > 0) {
                ...
                assertTrue("Should have depedencies.", rs.next());
                assertEquals("Incorrect depedenies count.", expected.getAllDependencies().length, rs.getInt(1));
            }
    ...
    private void assertCriteria(Phase expected) throws Exception {
        ...
        try {
            ...
            assertEquals("Incorrect criterias count.", expected.getAttributes().size(), count);
    ...
    protected void assertDatabase(Dependency expected) throws Exception {
        ...
        try {
            ...
            rs = pstmt.executeQuery();
            assertTrue("No such depedency", rs.next());
            assertEquals("Incorrect dependecy start value.", expected.isDependencyStart(), rs
            ...
            assertEquals("Incorrectlag time", expected.getLagTime(), rs.getLong("lag_time"));
        } finally {
            rs.close();
            pstmt.close();
        }
    ...
    protected void assertEquals(Phase expected, Phase actual) throws Exception {
        ...
        assertEquals("Differen depedencies size", expectedDeps.length, actualDeps.length);
        assertEquals("Incorrect criteria", expected.getAttributes(), actual.getAttributes());
    }

    /**
     * Compares the two Depedencies.
     *
    ...
    protected void assertEquals(Dependency expected, Dependency actual) throws Exception {
        ...
        assertEquals("Incorrect depedency start.", expected.isDependencyStart(), actual.isDependencyStart());
    ...
    /**
     * Checks if Phase and its depedencies were removed from database.
     *
