...
/**
 * <p>A failure test for {@link UploadTypeLookupUtility} class. Tests the proper handling of invalid input data by the
 * methods. Passes the invalid arguments to the methods and expects the appropriate exception to be thrown.</p>
 *
 * @author isv
 * @version 1.0
 */
public class UploadTypeLookupUtilityFailureTest extends AbstractTestCase {

    /**
     * <p>Gets the test suite for {@link UploadTypeLookupUtility} class.</p>
     *
     * @return a <code>TestSuite</code> providing the tests for {@link UploadTypeLookupUtility} class.
     */
    public static Test suite() {
        return new TestSuite(UploadTypeLookupUtilityFailureTest.class);
    }
    ...
    /**
     * <p>Failure test. Tests the {@link UploadTypeLookupUtility#lookUpId(Connection,String)} method for proper handling
    ...
    public void testLookUpId_Connection_String_connection_null() {
        try {
            UploadTypeLookupUtility.lookUpId(null, "Test Case");
    ...
    public void testLookUpId_Connection_String_value_null() {
        try {
            UploadTypeLookupUtility.lookUpId(TestDataFactory.getConnection(), null);
            Assert.fail("IllegalArgumentException should have been thrown");
        } catch (IllegalArgumentException e) {
            // expected behavior
        } catch (Exception e) {
    ...
    public void testLookUpId_Connection_String_value_ZERO_LENGTH_STRING() {
        try {
            UploadTypeLookupUtility.lookUpId(TestDataFactory.getConnection(), TestDataFactory.ZERO_LENGTH_STRING);
            Assert.fail("IllegalArgumentException should have been thrown");
        } catch (IllegalArgumentException e) {
            // expected behavior
        } catch (Exception e) {
    ...
    public void testLookUpId_Connection_String_value_WHITESPACE_ONLY_STRING() {
        try {
            UploadTypeLookupUtility.lookUpId(TestDataFactory.getConnection(), TestDataFactory.WHITESPACE_ONLY_STRING);
            Assert.fail("IllegalArgumentException should have been thrown");
        } catch (IllegalArgumentException e) {
            // expected behavior
        } catch (Exception e) {
    ...
    public void testLookUpId_Connection_String_value_UNKNOWN_LOOKUP_VALUE() {
        try {
            UploadTypeLookupUtility.lookUpId(TestDataFactory.getConnection(), TestDataFactory.UNKNOWN_LOOKUP_VALUE);
    ...
    /**
     * <p>Failure test. Tests the {@link UploadTypeLookupUtility#lookUpId,Connection,String} for proper behavior if the
    ...
    public void testLookUpId_Connection_String_SQLError() {
        ...
        try {
            UploadTypeLookupUtility.lookUpId(TestDataFactory.getMockConnection(), "Test Case");
            Assert.fail("SQLException should have been thrown");
        } catch (SQLException e) {
            // expected behavior
        } catch (Exception e) {
