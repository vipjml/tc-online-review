...
/**
 * <p>A failure test for {@link NotificationTypeLookupUtility} class. Tests the proper handling of invalid input data by
 * the methods. Passes the invalid arguments to the methods and expects the appropriate exception to be thrown.</p>
 *
 * @author isv
 * @version 1.0
 */
public class NotificationTypeLookupUtilityFailureTest extends AbstractTestCase {

    /**
     * <p>Gets the test suite for {@link NotificationTypeLookupUtility} class.</p>
     *
     * @return a <code>TestSuite</code> providing the tests for {@link NotificationTypeLookupUtility} class.
     */
    public static Test suite() {
        return new TestSuite(NotificationTypeLookupUtilityFailureTest.class);
    }
    ...
    /**
     * <p>Failure test. Tests the {@link NotificationTypeLookupUtility#lookUpId(Connection,String)} method for proper
    ...
    public void testLookUpId_Connection_String_connection_null() {
        try {
            NotificationTypeLookupUtility.lookUpId(null, "Timeline Notification");
    ...
    public void testLookUpId_Connection_String_value_null() {
        try {
            NotificationTypeLookupUtility.lookUpId(TestDataFactory.getConnection(), null);
            Assert.fail("IllegalArgumentException should have been thrown");
        } catch (IllegalArgumentException e) {
            // expected behavior
        } catch (Exception e) {
    ...
    public void testLookUpId_Connection_String_value_ZERO_LENGTH_STRING() {
        try {
            NotificationTypeLookupUtility.lookUpId(TestDataFactory.getConnection(), TestDataFactory.ZERO_LENGTH_STRING);
            Assert.fail("IllegalArgumentException should have been thrown");
        } catch (IllegalArgumentException e) {
            // expected behavior
        } catch (Exception e) {
    ...
    public void testLookUpId_Connection_String_value_WHITESPACE_ONLY_STRING() {
        try {
            NotificationTypeLookupUtility.lookUpId(TestDataFactory.getConnection(),
    ...
    /**
     * <p>Failure test. Tests the {@link NotificationTypeLookupUtility#lookUpId,Connection,String} for proper behavior
     * if the underlying service throws an unexpected exception.</p>
     *
    ...
    public void testLookUpId_Connection_String_SQLError() {
        ...
        try {
            NotificationTypeLookupUtility.lookUpId(TestDataFactory.getMockConnection(), "Timeline Notification");
            Assert.fail("SQLException should have been thrown");
        } catch (SQLException e) {
            // expected behavior
        } catch (Exception e) {
