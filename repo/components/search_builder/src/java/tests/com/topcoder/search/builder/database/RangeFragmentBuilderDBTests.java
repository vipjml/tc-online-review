 ...
 * @version 1.3
 */
public class RangeFragmentBuilderDBTests extends TestCase {
    /**
     * The namespace.
     */
    ...
    public void testbuildSearch_accuracy1() throws Exception {
        ...
        assertEquals("the generated String should be same.", "name > ?",
    ...
    public void testbuildSearch_accuracy5() throws Exception {
        filter = new BetweenFilter("name", "100", "1");
        builder.buildSearch(filter, searchContext);
        assertEquals("the generated String should be same.", "name >= ? AND name <= ?",
                searchContext.getSearchString().toString());
    }
}
