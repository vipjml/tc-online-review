...
/**
 * <p>An accuracy test for {@link com.topcoder.search.builder.database.OrFragmentBuilder} class. Tests the methods for proper
...
public class OrFragmentBuilderAccuracyTest extends TestCase {
    ...
    private static Pattern SEARCH_STRING_PATTERN
        = Pattern.compile("\\(\\(attr1=1\\)[ ]*OR[ ]*\\(attr2=2\\)[ ]*OR[ ]*\\(attr3=3\\)[ ]*OR[ ]*\\(attr4=4\\)[ ]*OR[ ]*\\(attr5=5\\)\\)");

    /**
     * <p>The instances of {@link OrFragmentBuilder} which are tested. These instances are initialized in
    ...
    public void testBuildSearch_Filter_SearchContext() throws Exception {
        for (int i = 0; i < this.testedInstances.length; i++) {
            ...
            Assert.assertEquals("The parameters are not collected correctly", 5, bindParameters.size());
            for (int j = 0; j < bindParameters.size(); j++) {
                Assert.assertEquals("The bind parameters are not collected correctly",
                                    String.valueOf(j + 1), bindParameters.get(j));
            }
        }
    }
}
