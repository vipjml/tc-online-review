...
public class OrFilterTests extends TestCase {
    ...
    /**
     * test the construct of OrFilter.
     *
     */
    public void testconstruct1() {
        assertNotNull("can not construct the instance of OrFilter", orFilter);
    }
    ...
    public void testisValid2() {
        try {
            orFilter.isValid(validators, null);
            fail("IllegalArgumentException should be throw");
        } catch (IllegalArgumentException e) {
            //success
        }
     ...
     * validators is empty,IllegalArgumentException should be thrown.
     *
     */
    public void testisValid4() {
        try {
            ValidationResult result = orFilter.isValid(new HashMap(), alias);
    ...
    public void testgetFilterType() {
        assertEquals("The getFilterType should return Filter.AND_FILTER",
            Filter.OR_FILTER, orFilter.getFilterType());
    }
    ...
    public void testclone() {
        ...
        assertEquals("The filter List  size should be the same with the clone",
            cl.filters.size(), orFilter.filters.size());
    }
}
