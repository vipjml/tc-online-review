...
/**
 * <p>
 * This class contains the stress unit tests for the BetweenFilter. change according to 1.3.
 * </p>
 *
 * @author PE, brain_cn
 * @version 1.3
 */
public class BetweenFilterStressTest extends TestCase {
    ...
    public static Test suite() {
        return new TestSuite(BetweenFilterStressTest.class);
    }
     ...
     * @throws Exception to JUnit
     */
    public void testBetweenFilterStressOfDatabaseConnectionStrategy()
        throws Exception {
        BetweenFilterThread[] threads = new BetweenFilterThread[THREAD_COUNT];
        ...
        Filter filter = new BetweenFilter("price", new Integer(4), new Integer(1));

        for (int i = 0; i < threads.length; i++) {
            threads[i] = new BetweenFilterThread();
        ...
        System.out.println("BetweenFilterStressOfDatabaseConnectionStrategy() tested: " +
        ...
        for (int i = 0; i < threads.length; i++) {
            ...
            assertEquals("validation of result should be ok", 4, sum);
        }
    }
     ...
     * @throws Exception to JUnit
     */
    public void testBetweenFilterStressOfLDAPConnectionStrategy()
        throws Exception {
        BetweenFilterThread[] threads = new BetweenFilterThread[THREAD_COUNT];
        ...
        Filter filter = new BetweenFilter("sn", "5", "3");
        ...
        long dure = System.currentTimeMillis() - startTime;
        System.out.println("testBetweenFilterStressOfLDAPConnectionStrategy() tested: " +
            Integer.toString(THREAD_COUNT) + " threads, " + Long.toString(dure) + " ms.");
...
class BetweenFilterThread extends Thread {
