...
/**
 * <p>
 * This class contains the stress unit tests for the LessThanFilter. change according to 1.3.
 * </p>
 *
 * @author PE, brain_cn
 * @version 1.3
 */
public class LessThanFilterStressTest extends TestCase {
    ...
    public static Test suite() {
        return new TestSuite(LessThanFilterStressTest.class);
    }
     ...
     * @throws Exception to JUnit
     */
    public void testLessThanFilterStressOfDatabaseConnectionStrategy()
        throws Exception {
        LessThanFilterThread[] threads = new LessThanFilterThread[THREAD_COUNT];
        SearchBundle sb = new SearchBundleManager(StressTestHelper.NAMESPACE).getSearchBundle(BUNDLE_NAME1);
        Map fields = new HashMap();
        fields.put("seller", StringValidator.hasLength(IntegerValidator.inRange(0, 20)));
        sb.setSearchableFields(fields);

        Filter filter = new LessThanFilter("seller", "c");

        for (int i = 0; i < threads.length; i++) {
            threads[i] = new LessThanFilterThread();
        ...
        System.out.println("LessThanFilterStressOfDatabaseConnectionStrategy() tested: " +
     ...
     * @throws Exception to JUnit
     */
    public void testLessThanFilterStressOfLDAPConnectionStrategy()
        throws Exception {
        LessThanFilterThread[] threads = new LessThanFilterThread[THREAD_COUNT];
        ...
        Filter filter = new LessThanFilter("sn", "3");
        ...
        long dure = System.currentTimeMillis() - startTime;
        System.out.println("testLessThanFilterStressOfLDAPConnectionStrategy() tested: " +
            Integer.toString(THREAD_COUNT) + " threads, " + Long.toString(dure) + " ms.");
...
class LessThanFilterThread extends Thread {
