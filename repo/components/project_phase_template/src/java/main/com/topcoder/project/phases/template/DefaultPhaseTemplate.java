...
/**
 * <p>
 * <code>DefaultPhaseTemplate</code> is a default implementation of <code>{@link PhaseTemplate}</code>
 * interface.
 * </p>
 * <p>
 * It manages two underlying variables - persistence(of type <code>{@link PhaseTemplatePersistence}</code>)
 * which is used as the actual template storage logic; and startDateGenerator(of type
 * <code>{@link StartDateGenerator}</code>) which is used as the default project start date generation logic
 * if there's no specific start date provided during the phase generation. By this, we can easily change the
 * persistence and start date generator implementations so that the client is able to swap out for different
 * storage and start date generation strategies without code changes - all we need to do is adding new
 * <code>{@link PhaseTemplatePersistence}</code> and/or <code>{@link StartDateGenerator}</code>
 * implementations.
 * </p>
 * <p>
 * <code>DefaultPhaseTemplate</code> can be created from configuration, please consult the component
 * specification and sample configuration file(docs/Default_Phase_Template.xml) for configuration details. It
 ...
 * </p>
 * <p>
 * <strong>Thread-safety:</strong> There are mutable inner variables in this class, but the access to these
 * variables are properly synchronized, so this class is thread-safe.
 * </p>
 * <p>
...
public class DefaultPhaseTemplate implements PhaseTemplate {
    ...
    /**
     * <p>
     * Represents the underlying template persistence from which the project phases will be generated.
     * </p>
     * <p>
     ...
     *             if the namespace is unknown or there's any errors in the
    ...
    public DefaultPhaseTemplate(String namespace) throws ConfigurationException {
        ...
        try {
            ...
            // retrieve "object_specification_namespace", "object_key" and
            // "object_identifier"
            ...
            throw new ConfigurationException("Object factory error occured while initializing the workdays.",
                e);
    ...
    /**
     * <p>
     * Create a <code>DefaultPhaseTemplate</code> with the given persistence , startDateGenerator and
     * workdays.
     * </p>
     *
     * @param persistence
     *            the PhaseTemplatePersistence instance to store the templates
     * @param startDateGenerator
     *            the StartDateGenerator instance to generate the project start
     *            date
     * @param workdays
     *            the Workdays instance used to create the project
     * @throws IllegalArgumentException
     *             if persistence or startDateGenerator or workdays is null
     */
    ...
    private void adjustPhases(Project project, long varPhaseId, long fixedPhaseId, Date fixedPhaseStartDate)
        ...
        if (diff == 0) {
            // already achieve the fixed, just return
            return;
        }
        if (diff < 0) {
            diff = -diff;
        ...
        long oldLength = varPhase.getLength() / MILLISECONDS_A_MINUTE;
        long left = 0, right = oldLength + MINUTES_A_WEEK + diff;

        // Use the binary search to find the best correction.
        // The search range is from 0 to oldLength + 7*24*60 + diff
        // this will take less than 25 iterations
        while (left <= right) {
            long mid = (left + right) / 2;
            varPhase.setLength(mid * MILLISECONDS_A_MINUTE);
            // get the fixed started date and calculate the difference again
            diff = (fixedPhaseStartDate.getTime() - fixedPhase.calcStartDate().getTime())
                / MILLISECONDS_A_MINUTE;
            if (diff == 0) {
                // fixed achieved
                return;
        ...
        // fail to achieve the fixed, throw the exception
        throw new PhaseGenerationException("The start date of phase " + fixedPhaseId + " could not be fixd.");
    }
