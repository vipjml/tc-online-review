...
/**
 * <p>
 * Unit tests for the {@link com.cronos.onlinereview.deliverables.SubmitterCommentDeliverableChecker} class.
 * </p>
 *
 * @author kr00tki
 * @version 1.0
 */
public class SubmitterCommentDeliverableCheckerTest extends DbTestCase {

    /**
     * The SubmitterCommentDeliverableChecker instance to test on.
     */
    private SubmitterCommentDeliverableChecker checker = null;
    ...
    protected void setUp() throws Exception {
        super.setUp();
        checker = new SubmitterCommentDeliverableChecker(getConnectionFactory(), CONNECTION_NAME);
    }

    /**
     * <p>
     * Tests the {@link SubmitterCommentDeliverableChecker#SubmitterCommentDeliverableChecker(DBConnectionFactory)}
    ...
    public void testConstructor_Factory() throws Exception {
        checker = new SubmitterCommentDeliverableChecker(getConnectionFactory());
     ...
     * <p>
     * Tests the
     * {@link SubmitterCommentDeliverableChecker#SubmitterCommentDeliverableChecker(DBConnectionFactory, String)}
     * constructor accuracy. Checks if internal fields are properly set.
     * </p>
     *
     * @throws Exception to JUnit.
    ...
    public void testConstructor_FactoryString_NullName() throws Exception {
        checker = new SubmitterCommentDeliverableChecker(getConnectionFactory(), null);
        assertSame("Incorrect connection factory.", getConnectionFactory(), getFieldFromBaseClass(checker,
                "connectionFactory"));
        assertNull("Name should be null.", getFieldFromBaseClass(checker, "connectionName"));
    }
    ...
    public void testConstructor_FactoryString_NullFactory() {
        try {
            new SubmitterCommentDeliverableChecker(null, CONNECTION_NAME);
            fail("Null connection factor, IAE expected.");
        } catch (IllegalArgumentException ex) {
            // ok
        }
    ...
    public void testConstructor_FactoryString_EmptyName() throws Exception {
        try {
            new SubmitterCommentDeliverableChecker(getConnectionFactory(), " ");
    ...
    /**
     * Tests the {@link SubmitterCommentDeliverableChecker#fillInQueryParameters(Deliverable, PreparedStatement)}
    ...
    public void testFillInQueryParameters() throws Exception {
        ...
        checker.fillInQueryParameters(new Deliverable(2, 1, 1, new Long(5), true), proxy.getProxy());

        assertEquals("Incorrect submission_id", new Long(5), proxy.getValue(0));
        assertEquals("Incorrect resource_id", new Long(1), proxy.getValue(1));
    }

    /**
     * Tests the {@link SubmitterCommentDeliverableChecker#getSqlQuery()} method. Checks if it returns non-null and
    ...
    /**
     * Tests the {@link SubmitterCommentDeliverableChecker#check(Deliverable)} method accuracy.
     *
     * @throws Exception to JUnit.
     */
    ...
    public void testCheck_Rejected() throws Exception {
        createReview(10, 1, 1, true);
        createReviewComment(1, SUBMITTER_COMMENT, 1, REJECTED, 10);
        Deliverable deliverable = new Deliverable(1, 1, 1, new Long(1), true);
        checker.check(deliverable);
        assertNotNull("The deliverable should be completed.", deliverable.getCompletionDate());
    }
    ...
    /**
     * Tests the {@link SubmitterCommentDeliverableChecker#check(Deliverable)} method failure.
     *
     * @throws Exception to JUnit.
     */
    ...
    /**
     * <p>
     * Tests the {@link SubmitterCommentDeliverableChecker#check(Deliverable)} method failure. Checks if
